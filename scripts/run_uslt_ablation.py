...
parser.add_argument("--ablation_no", type=int, default=0, choices=[0,1,2,3,4])
args = parser.parse_args()
gpu_device = args.gpu_device
ablation_no = args.ablation_no
print("gpu_device: ", gpu_device)
print("ablation_no: ", ablation_no)
...
#print("Complex check: complex words formed \n")
#print(complex_words)
...
def substition_ranker(suggestion_dictionary,complex_words,weight_bert,weight_cos,weight_lm,original_text,model,tokenizer,weight_freq,weight_len):
        print("substition_ranker")
        print("bert_weight: ", weight_bert)
        print("lm_weight: ", weight_lm)
        print("cos_weight: ", weight_cos)
        print("freq_weight: ", weight_freq)
        print("len_weight: ", weight_len)
        ...
        for key in suggestion_dictionary:
                ...
                for sugg in suggestion_dictionary[key]:
                        ...
                        #print(f"sugg[0]: {sugg[0]}")
                        #print(f"sugg[1]: {sugg[1]}")
                        #print(f"score: {score}")
...
if __name__ == '__main__':
        ...
        #target_path = "../data/supreme_org_test.txt"

        #for i in range(2):
        print(f"Ablation iteration {ablation_no}")
        target_path = "../data/supreme_org_test.txt"
        print("target_path", target_path)
        
        component_weight = [3.00, 0.36, 1.42, 2.00, 4.61]
        #component_weight = [1,2,3,4,5]
        component_weight[ablation_no] = 0
        bert_weight, lm_weight, cos_weight, freq_weight, len_weight = component_weight
        print("bert_weight: ", bert_weight)
        print("lm_weight: ", lm_weight)
        print("cos_weight: ", cos_weight)
        print("freq_weight: ", freq_weight)
        print("len_weight: ", len_weight)
#
#            simple_path = "../files/uslt_noss_yeni_deneme_supreme_test.txt"
...
#            freq_weight = 2.00
#            len_weight = 4.61
# one gets 0'd out each iteration to simulate the deficiency of each component
        if 0==1:
                simple_path = f"../files/uslt_noss_sıfır_deneme_supreme_test.txt"

        else:

                if bert_weight == 0:
                        ablation_ext = "wo_bert"
                elif lm_weight == 0:
                        ablation_ext = "wo_lm_loss"
                elif cos_weight == 0:
                        ablation_ext = "wo_cos_sim"
                elif freq_weight == 0:
                        ablation_ext = "wo_freq_feat"
                elif len_weight == 0:
                        ablation_ext = "wo_word_len_feat"

        simple_path = f"../files/uslt_noss_{ablation_ext}_supreme_test.txt"
